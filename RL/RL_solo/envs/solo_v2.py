...
class SoloEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def _get_obs(self):
        ...
        #    position = position[1:]
            #position = np.concatenate((position[:9], position[12:15]))
    ...
    def _compute_reward(self):
        ...
        ### (sin(alpha/2)*x,sin(alpha/2)*y,sin(alpha/2)*z,cos(alpha/2))
        ### goal: (sin(pi/2)*x,sin(pi/2)*y,sin(pi/2)*z,cos(pi/2))
        #mujoco_py.functions.mju_quat2Mat(res, quat)
        #res = res.reshape(3,3)
        #print(quat)
        #print(res)
        
        #zvel_reward = self.sim.data.qvel[4]>0 * 100
